	...
	<div id="content" style="background: rgba(0, 0, 0, 0)">
		...
		<div>
			...
			<h3 style="color: white;">Danh Sách Bài Đăng Cho Thuê Của Thành
				Viên</h3>
			<form action="<c:url value='/admin/list-unapproved-post'/>" id="formSubmit"
				method="get">
				<table id="danhmuc">
					<tr style="width: 100%">
						<th>Mã bài viết</th>
					...
					<c:forEach items="${model.postNewsEntities}" var="postNews">
					<c:if test="${postNews.status==0}">
						<tr>
							<td>${postNews.id}</td>
							<td>${postNews.title}</td>
							<td>${postNews.news.name}</td>
							<td>${postNews.type.name}</td>
							<td>${postNews.start_day}</td>
							<td>${postNews.end_day}</td>
							<td>${postNews.user.username }</td>
							<td>Chưa duyệt</td>
							<td><span><a
									href="view-detail-post-news?id=${postNews.id}"><i
										class="fas fa-eye">Xem chi tiết</i></a></span> <span><a
									href="article-approval?id=${postNews.id}"> <i
										class="fas fa-check">Duyệt</i></a></span> <span><a
									href="delete-post-news?id=${postNews.id}"><i
