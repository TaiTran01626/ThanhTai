...
<body
			...
			<p>
				<i style="padding-left: 53px; float: right; color: #f7921d;"
					class="far fa-calendar-alt"><springmess:message
						code="label.timePost" />:${postNews.start_day}</i>
			</p>
		</div>
		<div class="single">
			...
			<div class="as">
					...
					<div
						...
						<p style="padding-left: 50px; font-size: 18px;">
							<i class="fas fa-money-bill"></i><strong style="color: #f7921d"><springmess:message
								...
								${postNews.detail.price} Triệu VNĐ </span>
						</p>
					</div>
					<div
						style="border: 2px solid #00c6d7; border-top-right-radius: 23px; color: white;">
						<p style="padding-left: 50px; font-size: 18px;">
							<i class="fab fa-laravel"></i><strong style="color: #f7921d"><springmess:message
								...
								${postNews.detail.area}m<sup>2</sup>
							</span>
						</p>
					</div>
					<div
				...
				<div class="top-content"
					...
					<div class="white"
						...
						<c:if test="${postNews.status==1}">
							<button onclick="browseArticles(${postNews.id})"
								id="duyetphanhoi"
								class="hvr-shutter-in-vertical hvr-shutter-in-vertical2 ">
								Duyệt phản hồi</button>
							<button onclick="deleteFeedback(${postNews.id})" id="xoabaiviet"
								class="hvr-shutter-in-vertical hvr-shutter-in-vertical2 ">
								Xóa bài viết</button>
						</c:if>
						<c:if test="${postNews.status == 2}">
							<button onclick="rehibilitate(${postNews.id})"
								id="phuchoibaiviet"
								class="hvr-shutter-in-vertical hvr-shutter-in-vertical2 ">
								Phục hồi bài viết</button>
							<button onclick="deleteFeedback(${postNews.id})" id="xoabaiviet"
								class="hvr-shutter-in-vertical hvr-shutter-in-vertical2 ">
								Xóa bài viết</button>
						</c:if>
						<c:if test="${postNews.status==0}">
							<button onclick="articleApproval(${postNews.id})"
								id="duyetbaiviet"
								class="hvr-shutter-in-vertical hvr-shutter-in-vertical2 ">
								Duyệt bài viết</button>
						</c:if>
	...
	<div class="a1">
		<div class="as1" style="border: 2px solid black; height: auto;">
			...
			<c:forEach items="${postNews.detail.images}" var="image">
				<div class="col-md-3 ">
					<div class="col-md">
	...
	<script type="text/javascript">
		function articleApproval(id) {
			var id = id;
			$.ajax({
						type : "GET",
						contentType : "application/json",
						url : "${pageContext.request.contextPath}/admin/article-approval",
						data : {
							id : id,
						},
						dataType : 'json',
	...
	<script type="text/javascript">
		function browseArticles(id) {
			var id = id;
			$.ajax({
						type : "GET",
						contentType : "application/json",
						url : "${pageContext.request.contextPath}/admin/browse-articles",
						data : {
							id : id,
						},
						dataType : 'json',
	...
	<script type="text/javascript">
		function rehibilitate(id) {
			...
			$.ajax({
						type : "GET",
						contentType : "application/json",
						url : "${pageContext.request.contextPath}/admin/rehibilitate",
						data : {
							id : id,
						},
						dataType : 'json',
	...
	<script type="text/javascript">
		function deleteFeedback(id) {
			...
			$.ajax({
						type : "GET",
						contentType : "application/json",
						url : "${pageContext.request.contextPath}/admin/delete-feedback",
						data : {
							id : id,
						},
						dataType : 'json',
		...
		function addViaAjax(id) {
			...
			$.ajax({
				...
				success : function(data) {
					if (data.validated) {
						alert(data.name);
						$("#luuthongtin").val("Đã Lưu"), $("#luuthongtin")
								.hind();
					} else {
						alert(data.name);
					}
		...
		function browseArticles(id) {
			...
			$
					.ajax({
						type : "GET",
						contentType : "application/json",
						url : "${pageContext.request.contextPath}/savecomment?id="
								+ id,
						data : comment,
						dataType : 'json',
						timeout : 100000,
