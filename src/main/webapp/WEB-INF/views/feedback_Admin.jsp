	...
	<div id="content" style="background: rgba(0, 0, 0, 0)">
		...
		<div>
			...
			<h3 style="color: white;">Danh Sách Bài Đăng Cho Thuê Của Thành
				Viên</h3>
			<form action="<c:url value='/admin/list-feedback'/>" id="formSubmit"
				method="get">
				<table id="danhmuc">
					<tr style="width: 100%">
					...
					<c:forEach items="${model.feedbackEntities}" var="feedback">
						<tr>
						<td>${feedback.id}</td>
						<td>${feedback.post.id}</td>
						<td>${feedback.post.title}</td>
						<td>${feedback.time}</td>
						<td>${feedback.post.start_day}</td>
						...
						<td>Hiển Thị</td>
							...
							<td><span><a href="view-detail-post-news?id=${feedback.id}"><i class="fas fa-eye">Xem
										chi tiết</i></a></span> <span>
